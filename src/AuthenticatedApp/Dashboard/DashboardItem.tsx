...
const DashboardItem = ({stockItem} : DisplayItemProps) => { 
    return (
        <Stack direction='row' sx={{ width: '100%' }}>
            ...
            <Stack justifyContent='center' sx={{ padding: '10px', width: '20%' }}>
                <Typography variant='subtitle2' sx={{ fontWeight: 'bold' }}>{stockItem.amount}</Typography>
                <Typography variant='caption'>stock level</Typography>
            </Stack>
            <Divider orientation='vertical' flexItem />
            <Stack justifyContent='center' sx={{ padding: '10px', width: '20%' }}>
                <Typography variant='subtitle2' sx={{ fontWeight: 'bold' }}>{Math.round(orderPoint(stockItem.demandRateForYear, stockItem.deliveryTime))}</Typography>
                <Typography variant='caption'>reorder level</Typography>
            </Stack>
            <Divider orientation='vertical' flexItem />
            <Stack justifyContent='center' sx={{ padding: '10px', width: '20%' }}>
                <Typography variant='subtitle2' sx={{ fontWeight: 'bold' }}>{amountToOrder(stockItem.demandRateForYear, stockItem.orderCost, stockItem.unitHoldingCost)}</Typography>
                <Typography variant='caption'>reorder quantity</Typography>
            </Stack>
            <Divider orientation='vertical' flexItem />
            <Stack justifyContent='center' sx={{ padding: '10px', width: '20%' }}>
                <Typography variant='subtitle2' sx={{ fontWeight: 'bold' }}>{stockItem.amount <= Math.round(orderPoint(stockItem.demandRateForYear, stockItem.deliveryTime)) ? 'reorder' : 'OK'}</Typography>
